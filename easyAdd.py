...
class AddOneTaskApp(QtWidgets.QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Add-1 Task")
        ...
        self.display_time = 2     # Total display time for the number
        self.digit_display_time = 750  # Time to display each digit in milliseconds
        self.pause_time = 250  # Pause time between digits in milliseconds
    ...
    def add_one(self, number):
        return ''.join(str((int(digit) + 1) % 10) for digit in number)
    ...
    def next_trial(self):
        if self.current_trial < self.trials:
            ...
            self.correct_answer = self.add_one(self.current_number)
